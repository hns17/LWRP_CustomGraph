...
/**
    @file   LightWeightCustomLitSubShader.cs
    @date   2019.06.10
    @author hns17(hns17.tistory.com)
    @brief  CustomLitMaster Node와 Shader를 연결한다.
*/

namespace Hns17.CustomNode
{
    class LightWeightCustomLitSubShader : ICustomLitSubShader
    {
        ...
        Pass m_ForwardPassCustomLit = new Pass()
        {
            ...
            {
                CustomLitMasterNode.PositionSlotId
            }
        };

        Pass m_ForwardPassCustomUnLit = new Pass()
        {
            Name = "LightweightForwardAndOutLine",
            PixelShaderSlots = new List<int>()
            {
                CustomLitMasterNode.ColorSlotId,
                CustomLitMasterNode.AlphaSlotId,
        ...
        public string GetSubshader(IMasterNode masterNode, GenerationMode mode, List<string> sourceAssetDependencyPaths = null)
        {
            ...
            {
                // LightWeightCustomLitSubShader.cs GUID
                sourceAssetDependencyPaths.Add(AssetDatabase.GUIDToAssetPath("4b3d1ff2228bb12418007a4f8b042536"));
            }

            //Shader Template
            var templatePath = GetTemplatePath("lightweightCustomLitForwardPass.template");
            var extraPassesTemplatePath = GetTemplatePath("lightweightCustomLitExtraPasses.template");
            ...
            var cslMasterNode = masterNode as CustomLitMasterNode;

            //Select Pass
            var pass = cslMasterNode.shadeType == CustomLitMasterNode.ShadeType.Lit 
                ? m_ForwardPassCustomLit : m_ForwardPassCustomUnLit;

            var subShader = new ShaderStringBuilder();
            subShader.AppendLine("SubShader");
            using (subShader.BlockScope())
            {
                var materialTags = ShaderGenerator.BuildMaterialTags(cslMasterNode.surfaceType);
                ...
                var materialOptions = ShaderGenerator.GetMaterialOptions(cslMasterNode.surfaceType, cslMasterNode.alphaMode, cslMasterNode.twoSided.isOn);
        ...
        {
            var basePath = "Assets/Hns17/CustomNode/Editor/MasterNode/CustomLit";
        ...
        static string GetShaderPassFromTemplate(string template, CustomLitMasterNode masterNode, Pass pass, GenerationMode mode, SurfaceMaterialOptions materialOptions)
        {
            // ----------------------------------------------------- //
            //                         SETUP                         //
            // ----------------------------------------------------- //
            ...
            if (masterNode.IsSlotConnected(CustomLitMasterNode.NormalSlotId))
                defines.AppendLine("#define _NORMALMAP 1");
            
            if (masterNode.IsSlotConnected(CustomLitMasterNode.AlphaThresholdSlotId))
                defines.AppendLine("#define _AlphaClip 1");

            if (masterNode.shadeType != CustomLitMasterNode.ShadeType.Lit)
                defines.AppendLine("#define _UnLitMode 1");
            ...
            resultPass = resultPass.Replace("${Culling}", masterNode.GetCullMode());
            resultPass = resultPass.Replace("${ZTest}", masterNode.GetDepthTest());
