...
/**
    @file   LightWeightLegacySubShader.cs
    @date   2019.06.05
    @author hns17(hns17.tistory.com)
    @brief  LegacyMaster Node와 Shader를 연결한다.
*/

namespace Hns17.CustomNode
{
    class LightWeightLegacySubShader : ILegaySubShader
    {
        ...
        Pass m_ForwardPassLambert = new Pass()
        {
            ...
            {
                LegacyMasterNode.PositionSlotId
            }
        };

        Pass m_ForwardPassHalfLambert = new Pass()
        {
            Name = "LightweightForward",
            PixelShaderSlots = new List<int>()
            {
        ...
        Pass m_ForwardPassBlinnPhong = new Pass()
        {
            Name = "LightweightForward",
            PixelShaderSlots = new List<int>()
            {
        ...
        public string GetSubshader(IMasterNode masterNode, GenerationMode mode, List<string> sourceAssetDependencyPaths = null)
        {
            ...
            {
                // LightWeightLegacySubShader.cs GUID
                sourceAssetDependencyPaths.Add(AssetDatabase.GUIDToAssetPath("dc62a7d717761b14b950c5783a8fca6e"));
            }

            //Shader Template
            var templatePath = GetTemplatePath("lightweightLegacyForwardPass.template");
            var extraPassesTemplatePath = GetTemplatePath("lightweightLegacyExtraPasses.template");
            ...
            var lagacyMasterNode = masterNode as LegacyMasterNode;
            
            //Select Pass
            var pass = lagacyMasterNode.shadeType != LegacyMasterNode.ShadeType.Lambert ?
                lagacyMasterNode.shadeType == LegacyMasterNode.ShadeType.HalfLambert ? m_ForwardPassHalfLambert : m_ForwardPassBlinnPhong : m_ForwardPassLambert;

            var subShader = new ShaderStringBuilder();
            subShader.AppendLine("SubShader");
            using (subShader.BlockScope())
            {
                var materialTags = ShaderGenerator.BuildMaterialTags(lagacyMasterNode.surfaceType);
                ...
                var materialOptions = ShaderGenerator.GetMaterialOptions(lagacyMasterNode.surfaceType, lagacyMasterNode.alphaMode, lagacyMasterNode.twoSided.isOn);
            ...
            subShader.Append("CustomEditor \"Hns17.CustomNode.LegacyMasterGUI\"");
        ...
        {
            var basePath = "Assets/Hns17/CustomNode/Editor/MasterNode/LegacyShader";
        ...
        static string GetShaderPassFromTemplate(string template, LegacyMasterNode masterNode, Pass pass, GenerationMode mode, SurfaceMaterialOptions materialOptions)
        {
            // ----------------------------------------------------- //
            //                         SETUP                         //
            // ----------------------------------------------------- //
            ...
            if (masterNode.IsSlotConnected(LegacyMasterNode.NormalSlotId))
                defines.AppendLine("#define _NORMALMAP 1");

            if (masterNode.shadeType == LegacyMasterNode.ShadeType.HalfLambert)
